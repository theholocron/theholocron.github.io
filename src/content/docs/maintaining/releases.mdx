...
If you want to verify that the release will match the above rules then run a dry run of the release script (`pnpm release -- --dry-run`) and verify that the output of the command will match the expectations of the version you expect.

## Release a Version

Once a version has been determined, run `pnpm release`. This will update the `version` key within the `package.json` and the `pnpm-lock.yaml`, as well as generate the CHANGELOG, commit using our conventions and create the tag. **DO NOT USE the `pnpm version` command!**
