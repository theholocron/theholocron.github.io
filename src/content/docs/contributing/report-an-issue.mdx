...
There are typically two sides to test, CSS and JS. For the CSS, you can can run `pnpm lint:css` to check for syntax errors that may cause issues. With regard to JS, the same applies; run `pnpm lint:js` to check syntax errors and `pnpm test:js` to test the JS components. You can also run linting and testing on both sides by running `pnpm lint` and `pnpm test` respectively.

<Steps>
	1. **Results**: If there is a fail in a particular test, include the name of the test, otherwise list as "Passed".
	2. **Line(s) of Code**: Definitely not a requirement, but doesn't hurt if you've already pin pointed the issue. 3.
